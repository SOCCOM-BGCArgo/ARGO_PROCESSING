...
%
% check value: 1.062308 umol kg-1
...
data=[lat(:)/90 sind(lon(:)) cosd(lon(:)) sind(doy(:)).*fsigmoid(:) cosd(doy(:)).*fsigmoid(:) temp(:) psal(:) doxy(:) pres(:)./2e4+1./((1+exp(-pres(:)./300)).^3)];

temporaire=fopen([basedir 'Fichier_poids_PO4_hidden_ascii17_17.sn'],'r');
poids=fscanf(temporaire,'%f',[3 inf]);
fclose(temporaire);
poids=poids(3,:)';

ne=9;   % Number of inputs
nc1=17; % Number of neurons of the first hidden layer
...
Moy  =load([basedir 'moy_PO4.dat']);
Ecart=load([basedir 'std_PO4.dat']);
