function [allInfo] = MBARI_floatCount_summary()
    ...
    %
    % This output could then be summarized at the bottom of each processing
    % email for to provide quick-review of our float status each day.
    %
    % This script (ideally would be turned into a function) would be called by "Loop_Argo_Float.m" and the output stored
    % into the email msg strings that get sent at the end of each processing.
    %
    %--------------------------------------------------------------------------
    ...
    function [apex,navis,solo,all] = floatCount_byProg(var)
    ...
    %For counting ACTIVE floats by program
    %INPUT(S): A program string such as 'SOCCOM' or 'GO-BGC'
    %OUTPUT(S): A count of ACTIVE apex, navis, solo, and combined total floats 
    function [apex,navis,solo,all] = floatCount_byProgActive(var)
        ...
        apex = length(dActive(matches(dActive(:,idx),var) & matches(dActive(:,iFLT),"APEX")));
        navis = length(dActive(matches(dActive(:,idx),var) & matches(dActive(:,iFLT),"NAVIS")));
        solo = length(dActive(matches(dActive(:,idx),var) & matches(dActive(:,iFLT),"SOLO")));
        all = length(dActive(matches(dActive(:,idx),var)));
    end
    
    %For counting ALL floats by region
    %INPUT(S): A program string such as 'NAtl' or 'NPac'
    %OUTPUT(S): A count of apex, navis, solo, and combined total floats
    function [apex,navis,solo,all] = floatCount_byRegion(var)
    ...
    %For counting ACTIVE floats by region
    %INPUT(S): A program string such as 'NAtl' or 'NPac'
    %OUTPUT(S): A count of ACTIVE apex, navis, solo, and combined total floats 
    function [apex,navis,solo,all] = floatCount_byRegionActive(var)
    ...
    [aProg,nProg,sProg,tProg] = cellfun(@floatCount_byProg,varsProg);
    ...
    [aReg,nReg,sReg,tReg] = cellfun(@floatCount_byRegion,varsReg);
    ...
    a1= sum(aProg([1 3 4 5 7 8])); %All apex
    a2 = sum(aProgActive([1 3 4 5 7 8])); %Active apex
    a3 = sum(nProg([1 3 4 5 7 8])); %Navis
    a4 = sum(nProgActive([1 3 4 5 7 8]));
    a5 = sum(sProg([1 3 4 5 7 8])); %Solo
    a6 = sum(sProgActive([1 3 4 5 7 8]));
    a7 = sum(tProg([1 3 4 5 7 8])); %All floats
    a8 = sum(tProgActive([1 3 4 5 7 8]));
    ...
    %Generate headers
    FltCntList= [FltCntList; ["PROGRAM", "APEX", "NAVIS", "SOLO", "TOTAL"]]; %First line is headers
    ...
    for i = [2 6] %2 nad 6 pertain to SOCCOM and GO-BGC, respectively
        ...
        FltCntList = [FltCntList; [varsProg(i)+":" aProg(i)+"("+aProgActive(i)+")", nProg(i)+"("+nProgActive(i)+")", sProg(i)+"("+sProgActive(i)+")", tProg(i)+"("+tProgActive(i)+")"]];
        FltCntList= [FltCntList; [" ", " ", " ", " ", " "]];
    end
    
    %Manually add 'OTHER' category of floats
    FltCntList = [FltCntList; ["OTHER:" a1+"("+a2+")", a3+"("+a4+")", a5+"("+a6+")", a7+"("+a8+")"]];
    FltCntList= [FltCntList; [" ", " ", " ", " ", " "]];
    FltCntList = [FltCntList; ["--------------------------------------------------", "", "", "", ""]];
    ...
    C5_spacing = arrayfun(@blanks, 10 - cellfun('length', FltCntList(:,5)), UniformOutput=false); %Solos counts
    ...
    allInfo = categories+apex+navis+solo+total;
