...
%Inputs for the email (COMMENT THIS OUT IF YOU DON'T WANT TO SEND AN EMAIL)
...
%Overwrite the timelag
% timelag = datetime("now")-hours(24);

%FOR TESTING ON THE WHOLE FLEET
% floatList = floatListAll;

%FOR TESTING ON A SPECIFIC DATE RANGE
% dateStart = datenum(datetime(2024,10,17,6,0,0));
% dateEnd = datenum(datetime(2024,10,23,6,0,0));
% floatList = readtable("\\atlas\Chem\ARGO_PROCESSING\DATA\CAL\MBARI_float_list.txt");
% floatList = floatList(floatList.tfDead==0 & ~isnat(floatList.x1stDate),:);
% floatList = sortrows(floatList(~ismissing(floatList.maxCycleProcDate),:), "maxCycleProcDate", "descend");
...
%FOR TESTING ON SPECIFIC FLFOATS
% wmoList = [3902557;5907054;5906540];
% idList = {'ua23596';'ua21977';'un1452'};
%
%OR INDIVIDUAL FLOATS (MUST FILL OUT BOTH WMO AND MBARI ID)
%
% wmoList = 1902497;
% idList = {'wn1560'};
...
for c = 1:length(wmoList)
    ...
    tests.badO2Test = (ismember(wmoList(c),badCycles.O_wmo) & ~isempty(BSLCycle_oxygen)) | tests.MUXERfloat;
    ...
    if all([tests.nitrateColTest; tests.nitrateBSLtest])
        ...
        nitrateDATA = RTQC_tests(DATA, 5, DATA.qcDepth_nitrate, DATA.cycleRange_nitrate, tests.badO2Test, tests.goodDeepN, 1);
    ...
    %If not then display the reason it was terminated
        ...
        tests.NfailureTests = [tests.nitrateColTest==0 tests.nitrateBSLtest==0 tests.shallowFloatTest_nitrate==1];
    ...
    %RUN PH RTQC TESTS
    %===========================================================================================
    if all([tests.phColTest; tests.phBSLtest])
        disp('Processing PH')
        fprintf(msgLog,'Processing PH\n');
        % phDATA = realtimeQC(DATA, 3, tests.badO2Test);
        DATA.cycleRange_ph = [startCycle_ph endCycle_ph];
        phDATA = RTQC_tests(DATA, 3, DATA.qcDepth_ph, DATA.cycleRange_ph, tests.badO2Test, tests.goodDeepPH, 1);
    else
        failureMSG = {"Float has no PH"; "All future PH is on BSL"; "No PH data below 500m"};
        tests.PHfailureTests = [tests.phColTest==0 tests.phBSLtest==0 tests.shallowFloatTest_ph==1];
    ...
    if exist('phDATA', 'var')
        tests.noO2test_ph = ~tests.badO2Test & ~isempty(phDATA.noO2stns) && (phDATA.noO2stns(end)==totalStnsPH(end));
