...
for i = 1:length(SP)
    ...
    if c <=2 && i~=length(SP) && inds<=length(HRT)-2  %This eliminates imposing the n+1 indexing on the coarse resolution data at depth (n+1 would offset the Temp much more than 2m, this fix is not applicable at depth)
        %Temp
