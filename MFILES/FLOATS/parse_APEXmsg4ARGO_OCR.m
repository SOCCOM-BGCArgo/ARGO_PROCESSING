function data = parse_APEXmsg4ARGO_OCR(msg_file)
% PURPOSE:
%   This function parses an UW/MBARI APEX biochemical Argo *.msg file 
%   from a test float holding an OCR sensor and
%   returns a structure of raw data. The # of colums are determined from
%   the low resolution header line.  Note that a OCR data sampling is
%   returned along a seperate pressure axis -- future APEX floats with OCR
%   will have data returned along the lr_d pressure axis (along with other
%   BGC sensor data).
%
% USAGE:
%	data = parse_APEXmsg4ARGO_OCR(file_name)
%
% INPUTS:
...
%           data.ocr_hdr = ocean color radiometer section hdr
%           data.ocr_d  = ocean color radiometer data (separate pres axis!)
...
% EXAMPLES:
%   parse_APEXmsg4ARGO_OCR('c:\temp\7601.003.msg')
%   parse_APEXmsg4ARGO_OCR('7601.003.msg')
%
% CHANGES
...
% 07/26/22  - TM, added park-depth data extraction (based on Emily's
%               additions to the standard parsers)
...
ocr_res_format = '%f%f%08x%08x%08x%08x%08x';
ocr_ct         = size(regexp(ocr_res_format,'\%'),2);
...
while isempty(regexp(tline,'p\s+t\s+s','once')) % # find header line
    tline = fgetl(fid);
    if isnumeric(tline)
...
OCR_mode   = 1; %this is the ocr parser!
...
while ischar(tline)
    ...
    switch msg_task % do different tasks based on position in msg file
        ...
        case 'profile data' % EXTRACT DISCRETE SAMPLE DATA
            ...
            else % No high resolution data found - look for OCR
                msg_task = 'OCR data';
            end
        ...
        case 'OCR data'     % EXTRACT OCR DISCRETE SAMPLE DATA
            if isempty(regexp(tline,'^#','once')) % # means end of low res
                tmp = sscanf(tline,ocr_res_format);
                if size(tmp,1) == ocr_ct % MAKE SURE DATA LINE COMPLETE
                    ocr_res = [ocr_res; tmp'];
...
if isempty(low_res) && isempty(high_res)
    ...
    if ~isempty(ocr_res)
        data.ocr_d = [ocr_res(:,1) ocr_res(:,4:end)];  % p ch380 ch412 ch490 chPAR
        data.ocr_hdr = [{'p'};{'ocr380'};{'ocr412'};{'ocr490'};{'ocrPAR'}]; % These are labelled as "ch" in file -- is this the right order?
    else
        disp(['No ocr discrete data found for ',msg_file])
    end
    data.pk_d = pk_data;
