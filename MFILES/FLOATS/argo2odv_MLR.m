function tf_odv = argo2odv_MLR(WMO_ID, dirs, update_str, HR_flag)

% ************************************************************************
% PURPOSE:
%    This function creates ODV compaitble text files used in FloatViz
%    and SOCCOMViz using the *.mat profile files. If pH data exists and the
%    float is S of 45S ODV campatible files will be generated using Nancy
%    Williams MLR estimate for Alkalinity.
%
% USAGE:
...
if strcmp(update_str, 'update')
    ...
    clear WMO_path list float_names ind ind data_dir
    clear mat_files mat_cycles
    
    % NOW CHECK FLOATVIZ FILE
    fv_name = [WMO_ID, 'QC.TXT'];
    if exist([dirs.FVlocal,'MLR_QC\', fv_name],'file')
        old_d = get_FloatViz_data([dirs.FVlocal,'MLR_QC\', fv_name]);
        all_FV = unique(old_d.data(:,2));
        max_FV = max(old_d.data(:,2));
        
        %         if max_mat <= max_FV
        if N_mat == length(all_FV)
            fprintf(['MLR FloatViz file: %s is current ',...
                '(last cycle #: %03.0f)\r\n'], fv_name, max_FV);
            return
        end
    end
...
tLAT = rdata(:,4) <= -45;
tMLR = sum(tLAT) > 0; % 1 means there is data in range for MLR
if isempty(iPH) || ~tMLR % no pH or out of MLR range
    return
end
...
MLR_alk_str    = '';
tLAT = adj_data(:,iL) <= -45;
tMLR = sum(tLAT) > 0; % 1 means there is data in range
if ~tMLR
    ipH = []; % set to empty since float not in MLR range
end
...
if ~isempty(iPH) %  empty will be false
    ...
    % ************************************************************************
    % CALCULATE WILLIAMS MLR ALKALINITY FOR THE ADJUSTED DATA SET
    %              (ONLY for profiles South of 45S)
    % ************************************************************************
    % CHECK TO MAKE SURE FLOAT IS IN BOUNDS
    tLAT = adj_data(:,iL) <= -45;
    if sum(tLAT) > 0
        disp(['Calculating Alkalinity using Wiliams MLR for S of 45S ', ...
            '( Williams et al., 2017 )']);
        
        % ALKALINITY MLR COEFFICIENTS (optimized for South of 45S)
        ACoeff.cC   =  734.722320773799;   % Constant
        ACoeff.cST  = -15.4776417841503;   % Sigma Theta
        ACoeff.cO   = -0.111489142191378; % Oxygen umol/kg
        ACoeff.cS   =  59.7516385934224;   % Salinity
        ACoeff.cT   = -2.79046056637350;   % Temperature deg C
        ACoeff.cP   =  0.0134602966289807; % Depth (pressure I believe from FV file?)
        ACoeff.cGPA = -37.1209335266590;   % geo pot anom 5to1000dynm,dyn m
        
        % CALCULATE GEOPOTENTIAL ANOMALIES ( MODIFIED FROM NANCY WILLIAMS CODE )
        % loop through each cast
        % !! data set needs to go from surface to deep for each cast!!
        gpan50to1000 = ones(adj_r,1) * NaN; % predim output
        cycles = unique(adj_data(:,1)); % get casts to process
        ztol   = 7; % look up tol in meters for gpan1000 & gpan50
        for i = 1 : size(cycles) % step through casts
            tcast  = adj_data(:,1) == cycles(i); % index for subset & filling predim
            tmp    = adj_data(tcast,:); % subset
            % gpan units = [m^3 kg^-1 Pa == m^2 s^-2 == J kg^-1]
            gpan   = flip(sw_gpan(flip(tmp(:,iS)), flip(tmp(:,iT)), ...
                flip(tmp(:,iP)))); % geopotential anom relative to sea surf
            
            %         t1000m       = tmp(:,iP) > 960 & tmp(:,iP) < 1040; % ~1000m
            %         t50m         = tmp(:,iP) > 47  & tmp(:,iP) < 55;   % ~50m
            %         gpan1000     = nanmean(gpan(t1000m));
            %         gpan50       = nanmean(gpan(t50m));
            
            d1000m = abs(tmp(:,iP) - 1000);  % GET GPAN @ 1000 meters
            t1000m = d1000m == min(d1000m);
            if d1000m(t1000m) <= ztol
                gpan1000 = nanmean(gpan(t1000m));
            else
                gpan1000 = NaN;
                disp_str = sprintf(['Could not find gpan near 1000 meters', ...
                    ' for float ',info.MBARI_ID_str, ' cycle # %1.0f'],cycles(i));
                disp(disp_str)
            end
            
            d50m = abs(tmp(:,iP) - 50); % GET GPAN @ 50 meters
            t50m = d50m == min(d50m);
            if d50m(t50m) <= ztol
                gpan50 = nanmean(gpan(t50m));
            else
                gpan50 = NaN;
                disp_str = sprintf(['Could not find gpan near 50 meters', ...
                    ' for float ',info.MBARI_ID_str, ' cycle # %1.0f'],cycles(i));
                disp(disp_str)
            end
            
            gpan50to1000(tcast) = gpan1000 - gpan50;
        end
        
        
        % dynamic height in dynamic meters (1 dyn m = 10 m^2/sec^2)
        gpan50to1000dynm = gpan50to1000/10;
        gpan50to1000dynm(~tLAT) = NaN; % set any out of lat range = NaN
        clear  gpan50to1000 cycles tcast tmp t1000m t50m gpan gpan1000 gpan50
        clear tLAT
        
        Alk_Est = ACoeff.cC + ...
            ACoeff.cST  .* adj_data(:,iST) + ...
            ACoeff.cO   .* adj_data(:,iO) + ...
            ACoeff.cS   .* adj_data(:,iS) + ...
            ACoeff.cT   .* adj_data(:,iT) + ...
            ACoeff.cP   .* adj_data(:,iP) + ...
            ACoeff.cGPA .* gpan50to1000dynm;
        
        MLR_alk_str = ['TALK_MLR  = Alkalinity estimated from Multiple ', ...
            'Linear Regression algorithim for data south of ', ...
            '45S (Williams et al., 2017)'];
        ...
        adj_hdr  = [adj_hdr, 'MLR_ALK' 'MLR_ALK_QC'];
...
if ~isempty(iPH) % pH data exists
    ...
    if ~isempty(iAM) % LIAR ALKALINITY DIC
        PAR2     = adj_data(:,iAM);
        tQC_PAR2 = adj_data(:,iAM+1) ==4; %add check for bad ALK values, bogging CO2SYS down 10/9/2017
        PAR2(tQC_PAR2) = nan;
        ...
        MLR_PH25C =  OUT(:,18); % Col 19 in OUT is pH out
        ...
        for i = 1:size(cycles,1)
            ...
            tmp     = [adj_data(tcycle,[iP,iPH,iPH+1]), MLR_PH25C(tcycle)];
        ...
        MLR_pCO2 = OUT(:,19); % Col 19 in OUT is pCO2, (uatm)
        ...
        MLR_DIC  = OUT(:,2); % Col 2 in OUT is TCO2, (umol/kgSW)
        
        
        DIC_QF = max([adj_data(:,iS+1) adj_data(:,iO+1) ...
            adj_data(:,iAM+1) adj_data(:,iPH+1)],[],2);
        
        adj_hdr  = [adj_hdr, 'MLR PHTOT25C' 'MLR PHTOT25C_QC', ...
            'MLR_DIC' 'MLR_DIC_QC' 'MLR_pCO2' 'MLR_pCO2_QC'];
        adj_data = [adj_data, MLR_PH25C, DIC_QF, MLR_DIC, DIC_QF, ...
            MLR_pCO2, DIC_QF];
        [adj_r,adj_c] = size(adj_data); % get new size
        
        % DO A LAST CHECK FOR MISSING VALUES
        iPH25   = find(strcmp('MLR PHTOT25C',adj_hdr)   == 1);
        iDIC    = find(strcmp('MLR_DIC',adj_hdr)   == 1);
        iPCO2   = find(strcmp('MLR_pCO2',adj_hdr)   == 1);
...
CO2SYS_str = ['//NOTE ON CO2SYS CARBONATE SYSTEM CALCULATIONS:\r\n',...
	...
	'//This correction is not necessary for DIC and DIC is computed with the reported pH and the TALK_MLR value.\r\n'];	
...
ODV_adj(15,:) = {'pH25C[Total]'          '%0.4f' 'MLR PHTOT25C' '' '' ''};
ODV_adj(16,:) = {'TALK_MLR[µmol/kg]'     '%4.0f' 'MLR_ALK' '' '' ''};
ODV_adj(17,:) = {'DIC_MLR[µmol/kg]'      '%4.0f' 'MLR_DIC' '' '' ''};
ODV_adj(18,:) = {'pCO2_MLR[µatm]'       '%4.1f' 'MLR_pCO2' '' '' ''};
...
% OCR VARIABLES
ODV_adj(21,:) = {'DOWN_IRRAD380[W/m^2/nm]'            '%0.6f' 'DOWN_IRRADIANCE380' '' '' ''};
ODV_adj(22,:) = {'DOWN_IRRAD412[W/m^2/nm]'            '%0.6f' 'DOWN_IRRADIANCE412' '' '' ''};
ODV_adj(23,:) = {'DOWN_IRRAD490[W/m^2/nm]'            '%0.6f' 'DOWN_IRRADIANCE490' '' '' ''};
ODV_adj(24,:) = {'DOWNWELL_PAR[µmol Quanta/m^2/sec]'  '%0.6f' 'DOWNWELLING_PAR' '' '' ''};
% ************************************************************************
...
if sum(strcmp(ODV_adj{16,3},adj_hdr))
    ODV_adj(16,4:6) = {'' '' MLR_alk_str}; %TALK MLR
end
if sum(strcmp(ODV_adj{17,3},adj_hdr))
    str =['estimated with CO2SYS(TALK_MLR,pHinsitu) for Matlab ', ...
        '(see note below)'];
    ODV_adj(17,4:6) = {'' '' str}; %MLR DIC
end
if sum(strcmp(ODV_adj{18,3},adj_hdr))
    str =['estimated with CO2SYS(TALK_MLR,biased pHinsitu) for Matlab ', ...
        '(see note below)'];
...
if sum(strcmp(ODV_adj{21,3},raw_hdr))
...
if QC_check == 1
    % PRINT HEADER FIRST
    disp(['Printing adjusted data to: ',dirs.FVlocal, 'MLR_QC\', ...
        info.FloatViz_name,'QC', '.txt']);
    fid_adj  = fopen([dirs.FVlocal,'MLR_QC\', info.FloatViz_name,'QC','.TXT'],'W','n','UTF-8');
...
if QC_check == 1 && HR_flag == 1
    ...
    if strcmp(info.float_type, 'APEX') && ~tf_APEX_OCR
        ...
        disp(['Printing HR+LR adjusted data to: ',dirs.FVlocal,'MLR_QC\', ...
            info.FloatViz_name,'_HRQC', '.txt']);
        fid_adj  = fopen([dirs.FVlocal,'MLR_QC\', info.FloatViz_name,'_HRQC','.TXT'],'W','n','UTF-8');
        fprintf(fid_adj,'//0\r\n');
        fprintf(fid_adj,'//<Encoding>UTF-8</Encoding>\r\n');
        fprintf(fid_adj,['//File updated on ',datestr(now,'mm/dd/yyyy HH:MM'), ...
            '\r\n']);
