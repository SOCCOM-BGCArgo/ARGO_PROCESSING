function [wave dectime zenang IT ID IS] = radmod_jp(sdn, lat, lon, met)

% TEST



%function [wave dectime zenang IT ID IS QT QD QS Eo] = radmod2(dectime)
%  radmod
...
dvec = datevec(sdn);
dectime = sdn - datenum(dvec(1),1,0);
...
% realmet=1;
% if realmet==1
%   [dectime,lat,long,psp,slvp,dryt,relhum,windspd,windavg]=metload2(dectime);
% else % BUILD FAKE MAT DATA
%   [dectime,lat,long,psp,slvp,dryt,relhum,windspd,windavg]=metload('uv:fun305:henlop305:','met.dat');
% end

% *************************************************************************
% MAKE MET DATA - JP 10 Nov 2017
psp     = met.psp; % NOT USED!
slvp    = met.slvp;
dryt    = met.dryt;
relhum  = met.relhum;
windspd = met.windspd;
...
[zalt sorad] = soradna1(dectime-1, 2008, -lon, lat);
zenang = 90-zalt;
...
[M, MP, MOZ, MPST, OZ, precwat] = atmosph(lat,lon,dectime,zenang,slvp,dryt,relhum);
...
trans(lat,lon,wave,ah2o,ao3,ao2,viz,slvp,relhum,precwat,windspd,windavg,am,zenang,M,MP,MOZ,MPST,OZ);
...
%    QS = scattered (diffuse) spectral irradiance (mole/(m^2 s um))
[QT, IT, QD, QS, ID, IS] = ...
speclite(lat,lon,dectime,zenang,esd,wave,etr,ah2o,ao3,ao2,viz,slvp,relhum,precwat,windspd,windavg,am, ...
         TR,TA,TW,TO,TU,TAA,TAS,FS,RGD,RGS,RS);
