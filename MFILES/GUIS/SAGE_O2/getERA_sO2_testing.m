function ERA = getERA_sO2(inputdir,SDN,LON,LAT)
% ************************************************************************
% getERA_sO2.m
% ************************************************************************
%
...
for yr = start_yr:end_yr           % loop through each year-files
    ...
    if yr == NOW(1) %Current year will have an extra dimension ("expvar") which is length 2, and identifies the QC'd or NRT ERA.  We want to ignore this variable; the NRT is acceptable!
        ...
        if LON_flag ==1                     % No 0 meridian crossing data
            d = double(ds.data(dsv, [1 1 lat_ind(1) lon_ind(1)], ...
                [tlength 1 lat_ind(2) lon_ind(2)]));
            d = squeeze(d);
        else
            disp('PLOT TO VERIFY MERIDIAN CROSSING CODE!!!!');
            d1 = double(ds.data(dsv, [1 1 lat_ind(1) 1], ...
                [tlength 1 lat_ind(2) lon_ind(1)]));
            d2 = double(ds.data(dsv, [1 1 lat_ind(1) lon_ind(2)], ...
                [tlength 1 lat_ind(2) length(lon)]));
            d = cat(3,d1,d2);
            d = squeeze(d);
            clear d1 d2
        end
    ...
    else % Else any year other than current year!
        if LON_flag ==1                     % No 0 meridian crossing data
