function tf_odv = sprofmat2ODV(sprof_out, dirs)

% ************************************************************************
% PURPOSE: 
%    This function creates ODV compatitble text files used in FloatViz
%    and SOCCOMViz using the *Sprof NetCDF files. If pH data exists the
...
%
% INPUTS:
%   sprof_out = a a structure from a processed Sprof NetCDF file using
%               Spro2mat.m THIS IS RAW DATA - NO CORRECTIONS YET
%
...
dirs.txt = [user_dir,fp,'Documents',fp,'MATLAB',fp,'ARGO',...
            fp,'DATA',fp,'RAW',fp];
...
disp(['Printing raw data to: ',dirs.txt, 'ODV',strtrim(info.WMO(1,:)),'.TXT']);
fid_raw  = fopen([dirs.txt, 'ODV',strtrim(info.WMO(1,:)),'.TXT'],'W');  
