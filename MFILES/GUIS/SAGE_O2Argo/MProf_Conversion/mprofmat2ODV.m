function tf_odv = mprofmat2ODV(f_info, outputDIR)

% ************************************************************************
% PURPOSE: 
%    This function creates ODV compaitble text files used in FloatViz
%    and SOCCOMViz using the *Mprof NetCDF files. If pH data exists the
...
%
% INPUTS:
%   mprof_out = a a structure from a processed Mprof NetCDF file using
%               mpro2mat.m THIS IS RAW DATA - NO CORRECTIONS YET
...
%
%   outputDIR =  Either an empty variable or a string defining where the
%               ODV ascii file will be written.  If empty, default of 
%               C:/temp is used.

%
% CHANGE HISTORY
% 09/27/2017 - created by JP
% 03/20/2018 - T.Maurer, redefined function inputs and added
% file-separators.
...
% ************************************************************************
% SET UP OUTPUT DIRECTORY - default to C:/temp if user left undefined.
% ************************************************************************
fp = filesep;
if isempty(outputDIR)
    mytempfolder = [getenv('HOMEDRIVE'),fp,'temp',fp]; 
    if ~exist(mytempfolder, 'dir')
        mkdir(mytempfolder);
        disp([mytempfolder,' does not exist. Creating directory...'])
    end
    outputDIR = mytempfolder;
end
...
ODV_raw(6,:)  = {'Oxygen[µmol/kg]'       '%0.3f' 'DOXY' '' '' ''};   
ODV_raw(7,:)  = {'OxygenSat[%]'          '%0.3f' 'DOXY_%SAT' '' '' ''};
...
fprintf(fid_raw,'//This data has been sorted and binned. Any non-monotonic\r\n');
fprintf(fid_raw,'//profile behavior has been lost!!! Further flagged data\r\n');
fprintf(fid_raw,'//has been set to NaN (CHLA > 3, all other parameters > 2)\r\n');
fprintf(fid_raw,'//If PRES flag is bad, PRES and all associated data was removed\r\n');
fprintf(fid_raw,'//Binning was applied after these adjustmenst\r\n');
fprintf(fid_raw,'//\r\n');
