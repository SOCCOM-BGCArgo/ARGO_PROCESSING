function data = get_WOA2013_local_sO2Argo(track, depth_bnds, ocean_var,user_dir)
% PURPOSE: 
...
%
% USAGE:
%	data = get_WOA2013_local_sO2Argo(track, depth_bnds, ocean_var)
%
% INPUTS:
...
%              
% EXAMPLES:
%   data = get_WOA2013_local_sO2Argo(track, [0 1000], 'NO3')
%
...
if isempty(user_dir)
    user_dir = [getenv('USERPROFILE'),fp,'Documents',fp,'MATLAB',fp]; %returns user path,i.e. 
    disp('USER PATH WAS UNDEFINED...CHECK PATHS.  USING DEFUALT BASE PATH OF:')
    disp([getenv('USERPROFILE'),'\Documents\MATLAB\'])
end
data_site  = [user_dir,fp,'ARGO_PROCESSING',fp,'DATA',fp,'WOA2013',fp];
...
% LOCAL PATH
data_path = [data_site,WOA_info{var_ind,1},'\'];
fn_str     = regexprep(fn_str,'V',WOA_info{var_ind,2}); % build name: add var
fn_str     = regexprep(fn_str,'XXX',WOA_info{var_ind,4});% add data set
var_name  = [WOA_info{var_ind,2},'_an']; % variable to extract
